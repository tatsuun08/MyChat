...
class MainActivity : AppCompatActivity() {
    ...
    private fun sendMessage(text: String?, imageUrl: String?, isMe: Boolean) {
        messageList.add(Message(text, imageUrl, isMe))
    ...
    override fun onCreate(savedInstanceState: Bundle?) {//Bundleは入力途中の一時データ ?はNullの場合があるかもしれない
        ...
        sendButton.setOnClickListener {
            ...
            if (text.isNotEmpty()) {
                // リストにデータを追加
                messageList.add(Message(text, null, true))
                ...
                sendMessage(replyText, null, false)
//                val uniqueId = System.currentTimeMillis()
//                val testImageUrl = "https://picsum.photos/200?random=$uniqueId" // ランダムな画像が返るURL
//                sendMessage(null, testImageUrl, false)
        ...
        ViewCompat.setOnApplyWindowInsetsListener(mainLayout) { v, insets ->
            // システムバー（上部のステータスバーや下部のナビゲーションバー）の情報を取得
            val systemBars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
            // キーボード（IME）の情報を取得
            val imeInsets = insets.getInsets(WindowInsetsCompat.Type.ime())
            // 上の余白：ステータスバーの高さ分
            // 下の余白：キーボードが表示されていればその高さ、なければナビゲーションバーの高さ
